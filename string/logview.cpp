...
int main(int argc, char *argv[]) {
    ...
    //std::cout<<"// Error if there are not 3 things on the command line, passed"<<std::endl;
    if (argc != 3) { output_usage_and_exit(argv[0], opt); }

    // Open file, quit if open fails
    //std::cout<<"// Open file, quit if open fails, passed"<<std::endl;
    std::ifstream in(argv[2]);
    if (!in) { std::cerr << "Couldn't open " << argv[2] << "\n"; exit(2); }

    // Process the log file
    //std::cout<<"// Process the log file, passed"<<std::endl;
    ...
    //std::cout<<"handling the option, passed"<<std::endl;
